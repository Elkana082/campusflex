...
export default function Profile() {
  ...
  const socialLinks = [
    { key: "tiktok",    handle: profileUser.socials?.tiktok,    color: "#fe2c55", url: `https://tiktok.com/@${profileUser.socials?.tiktok}`,    label: "TikTok" },
    { key: "instagram", handle: profileUser.socials?.instagram,  color: "#c13584", url: `https://instagram.com/${profileUser.socials?.instagram}`,  label: "Instagram" },
    { key: "facebook",  handle: profileUser.socials?.facebook,   color: "#1877f2", url: `https://facebook.com/${profileUser.socials?.facebook}`,   label: "Facebook" },
  ].filter((s) => s.handle);
  ...
  return (
    <div style={{ paddingBottom: 90 }}>
        ...
        <div style={{ position: "absolute", bottom: -42, left: 16 }}>
          <div style={{
            ...
            background: profileUser.profilePicture
              ? `url(${profileUser.profilePicture}) center/cover`
              : "linear-gradient(135deg, #ede9fe, #dbeafe)",
            display: "flex", alignItems: "center", justifyContent: "center",
            fontSize: 32, fontFamily: "Syne", fontWeight: 800, color: "#7c3aed",
          }}>
      ...
      <div style={{ padding: "52px 16px 0" }}>
        ...
        <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", marginBottom: 8 }}>
          <div>
            <div style={{ display: "flex", alignItems: "center", gap: 6, flexWrap: "wrap" }}>
              <span className="syne" style={{ fontWeight: 800, fontSize: 20, color: "var(--text)" }}>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 6, marginTop: 3 }}>
              {profileUser.displayName && (
                <span style={{ fontSize: 13, color: "var(--muted)" }}>{profileUser.displayName}</span>
              )}
              {campus && (
                <span style={{ background: campus.color + "18", color: campus.color, border: `1px solid ${campus.color}33`, borderRadius: 20, padding: "1px 9px", fontSize: 11, fontWeight: 700 }}>
          ...
          <div style={{ display: "flex", gap: 8 }}>
            {isOwnProfile ? (
              <button
                ...
                style={{ background: "var(--surface)", border: "1.5px solid var(--border)", borderRadius: 12, padding: "8px 16px", color: "var(--text)", cursor: "pointer", fontSize: 13, fontWeight: 700 }}
              >
                Edit Profile
              </button>
            ) : (
              /* Message button — shown on other people's profiles */
              <button
                onClick={() => navigate(`/messages/${profileUser._id}?username=${profileUser.username}`)}
                style={{ background: "linear-gradient(135deg, var(--accent), var(--pink))", border: "none", borderRadius: 12, padding: "8px 16px", color: "#fff", cursor: "pointer", fontSize: 13, fontWeight: 700, display: "flex", alignItems: "center", gap: 6 }}
              >
        ...
        {/* Bio */}
        {profileUser.bio && (
          <p style={{ fontSize: 13, color: "var(--subtext)", lineHeight: 1.65, marginBottom: 12 }}>
            {profileUser.bio}
          </p>
        )}

        {/* Social links — only shown if filled in */}
        ...
        {profileUser.badges?.length > 0 && (
          <div style={{ display: "flex", gap: 6, flexWrap: "wrap", marginBottom: 14 }}>
            {profileUser.badges.map((b) => (
              <span key={b} style={{ background: "var(--accent)18", color: "var(--accent)", border: "1px solid var(--accent)33", borderRadius: 20, padding: "3px 11px", fontSize: 11, fontWeight: 700 }}>
                {b}
              </span>
            ))}
          </div>
        ...
        {/* Feature request — only own profile, non-admin */}
        {isOwnProfile && !isAdmin && (
          <button
            onClick={() => navigate("/feature-request")}
            style={{ width: "100%", background: "linear-gradient(135deg, var(--accent), var(--pink))", border: "none", borderRadius: 14, padding: "13px", color: "#fff", cursor: "pointer", fontSize: 14, fontWeight: 800, fontFamily: "Syne", marginBottom: 16 }}
          >
        ...
        {posts.length === 0 ? (
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 3 }}>
            {posts.map((p) => (
              <div key={p._id} style={{ aspectRatio: "1/1", background: "var(--border)", borderRadius: 4, overflow: "hidden", cursor: "pointer" }}>
                {p.mediaType === "video"
