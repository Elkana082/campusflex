...
router.post("/me/feature-request", protect, uploadPost.array("media", 4), async (req, res) => {
  try {
    ...
    const request = await FeatureRequest.create({
      ...
      media: mediaArray, caption: req.body.caption || "", status: "pending",
    });
...
router.post("/admin/feature-request/:id/approve", protect, adminOnly, async (req, res) => {
  try {
    ...
    const media = request.media?.length ? request.media : [{ url: request.mediaUrl, type: request.mediaType }];
    const post = await Post.create({
      author: request.user._id, campus: request.campus,
      mediaUrl: media[0].url, mediaType: media[0].type, media,
      caption: request.caption || `✨ Featured post by @${request.user.username}`,
      tags: ["Featured", "CampusFlex"], mentions: [], featured: true,
    });
    ...
    await Notification.create({
      recipient: request.user._id, type: "feature_approved", post: post._id,
      text: "🎉 Your feature request was approved! Your post is now live on the feed.",
    });
    res.json({ message: "Approved ✅ Post is now live!", post });
  } catch (err) {
    console.error(err);
    res.status(500).json({ message: "Server error" });
  }
...
router.post("/admin/feature-request/:id/reject", protect, adminOnly, async (req, res) => {
  try {
    ...
    if (!request) return res.status(404).json({ message: "Request not found" });
    await Notification.create({
      recipient: request.user._id, type: "feature_rejected",
...
// ── GET /:username — public profile — MUST be last ────────────────────────────
